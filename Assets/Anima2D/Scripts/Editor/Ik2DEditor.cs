...
namespace Anima2D 
{
	...
	public class Ik2DEditor : Editor
	{
		...
		SerializedProperty m_AlternateSourceTransformProperty;
		SerializedProperty m_AlternateSourceWeightProperty;
		...
		protected virtual void OnEnable()
		{
			...
			m_AlternateSourceTransformProperty = serializedObject.FindProperty("m_AlternateSourceTransform");
			m_AlternateSourceWeightProperty = serializedObject.FindProperty("m_AlternateSourceWeight");
		...
		override public void OnInspectorGUI()
		{
			...
			Transform alternateSourceTransform = null;
			
			if(m_Ik2D.alternateSourceTransform)
			{
				alternateSourceTransform = m_Ik2D.alternateSourceTransform;
			}
			
			EditorGUI.BeginChangeCheck();
			
			Transform newAlternateSourceTransform = EditorGUILayout.ObjectField(new GUIContent("Alternate Source"),alternateSourceTransform,typeof(Transform),true) as Transform;
			
			if(EditorGUI.EndChangeCheck())
			{
				Undo.RegisterCompleteObjectUndo(m_Ik2D,"set Alternate Source");

				if(newAlternateSourceTransform != alternateSourceTransform)
				{
					m_AlternateSourceTransformProperty.objectReferenceValue = newAlternateSourceTransform;					
					IkUtils.InitializeIk2D(serializedObject);
					EditorUpdater.SetDirty("set Alternate Source");
				}
			}
			...
			*/
			...
			EditorGUI.BeginChangeCheck();
			EditorGUILayout.Slider(m_AlternateSourceWeightProperty,0f,1f);
