	...
	public abstract class IkSolver2D
	{
		...
		public class SolverPose 
		{
			...
			public void RestoreDefaultPose()
			{
				if(bone)
				{
					bone.SetLocalRotation(defaultLocalRotation);
				}
			}
		}
		...
		void UpdateBones()
		{
			for(int i = 0; i < solverPoses.Count; ++i)
			{
				...
				if(solverPose != null && solverPose.bone)
				{
					if(weight == 1f)
					{
						solverPose.bone.SetLocalRotation(solverPose.solverRotation);
					}else{
						solverPose.bone.SetLocalRotation(Quaternion.Slerp(solverPose.bone.transform.localRotation,
